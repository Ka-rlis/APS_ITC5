...
void loop() {
  ...
  Serial.print("raw: "); 
  pwmMap(input(0,0));
  Serial.print(pwmMap(input(0,0)));
  Serial.print( "   " );
  Serial.print("raw2: ");
  pwmMap(input(1,0));
  Serial.print(pwmMap(input(1,0)));
  Serial.print( "   " );
  Serial.print( "yaw: " );
...
BLA::Matrix<6, 1> L_y = L * (y - (C * curr_state));
  ...
  u_e = -K_f * next_state; 
  u_e(0, 0) = (u_e(0, 0) > 1.0f) ? 1.0f : ((u_e(0, 0) < -1.0f) ? -1.0f : u_e(0, 0));
  u_e(1, 0) = (u_e(1, 0) > 1.0f) ? 1.0f : ((u_e(1, 0) < -1.0f) ? -1.0f : u_e(1, 0));
